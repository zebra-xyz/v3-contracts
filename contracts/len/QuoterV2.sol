...
contract QuoterV2 is IQuoterV2, IZebraV3SwapCallback, PeripheryImmutableState {
    using Path for bytes;
    using SafeCast for uint256;
    using PoolTicksCounter for IZebraV3Pool;
    ...
    function getPool(
        ...
        address tokenB,
        uint24 fee
    ) private view returns (IZebraV3Pool) {
        return IZebraV3Pool(PoolAddress.computeAddress(factory, PoolAddress.getPoolKey(tokenA, tokenB, fee)));
    }

    /// @inheritdoc IZebraV3SwapCallback
    function zebraV3SwapCallback(
        ...
        IZebraV3Pool pool = getPool(tokenIn, tokenOut, fee);
        ...
        )
    {
        ...
        IZebraV3Pool pool = getPool(params.tokenIn, params.tokenOut, params.fee);
